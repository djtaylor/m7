...
sub xml_tree	{ return shift->{_xml_tree}; }
...
sub xml2DB {
	...
	my $m7p_plan_check	= $m7p->db->selectcol_arrayref("SELECT * FROM plans WHERE plan_id='" . $m7p_plan_id . "'");
	if (@$m7p_plan_check) {
	...
	foreach (@m7_xml_files) {
	    my $m7p_xml_tree			= $m7p->lib_xml->parse_file($_);

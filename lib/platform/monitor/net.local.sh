		...
		"mtr")
			...
			do
				...
				if [ "$TEST_MTR_EXIT_CODE" != "0" ]; then
					...
					TEST_SUMMARY_BLOCK+="$(/usr/bin/perl ~/lib/perl/net/mtr_parse.pl $TEST_MTR_LOG)"
					TEST_SUMMARY_BLOCK+="\t\t</host>\n"
				fi
			done
...
if [ ! -z "$(sqlite3 ~/db/cluster.db "SELECT * FROM M7_Nodes WHERE Type='worker' AND Name='$(hostname -s)';")" ]; then
	...
	$TEST_DIRECTOR_USER@$TEST_DIRECTOR_IP_ADDR:~/results/$NM_TARGET_ID/$TEST_WORKER_NAME.xml >> $M7LOG_XFER 2>&1
	...
	mkdir ~/results/$NM_TARGET_ID
	mv $TEST_RESULT_FILE ~/results/$NM_TARGET_ID/$(hostname -s).xml
fi
